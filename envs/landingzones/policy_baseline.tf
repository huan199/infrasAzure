...
########################
variable "mg_id" {
  description = "Name của Management Group (Landing zones)"
  type        = string
  default     = "Langind_zones"   # đúng theo portal
}
...
# Landing Zones: CHẶN tạo Public IP resource
...
########################
# 2) Custom: Deny NIC có Public IP (scope MG)
########################
resource "azurerm_policy_definition" "deny_nic_with_public_ip" {
  ...
  management_group_id  = data.azurerm_management_group.this.id   # ✅ Đặt definition tại MG
...
########################
# 3) Custom: Deny tạo Public IP resource (scope MG)
########################
